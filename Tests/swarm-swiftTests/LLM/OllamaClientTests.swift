...
final class OllamaClientTests: XCTestCase {
    ...
    func testCreateChatCompletion() {
        ...
        request.appendMessages(messages: [
            Message().withRole(role: "user").withContent(content: "Hello! Please keep the response very short.")
        ])
        
        // Print the request payload
        if let requestData = try? request.rawData(),
           let requestJson = try? JSONSerialization.jsonObject(with: requestData, options: .fragmentsAllowed),
           let requestStr = try? JSONSerialization.data(withJSONObject: requestJson, options: .prettyPrinted) {
            print("\n=== Request to Ollama ===")
            print(String(data: requestStr, encoding: .utf8) ?? "")
        }
        ...
        client.createChatCompletion(request: request) { result in
            ...
            case .success(let response):
                ...
                if let responseData = try? response.rawData(),
                   let responseJson = try? JSONSerialization.jsonObject(with: responseData, options: .fragmentsAllowed),
                   let responseStr = try? JSONSerialization.data(withJSONObject: responseJson, options: .prettyPrinted) {
                    print(String(data: responseStr, encoding: .utf8) ?? "")
                }
