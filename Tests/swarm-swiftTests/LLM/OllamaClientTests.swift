...
class OllamaClientTests: XCTestCase {
    ...
    func testCreateChatCompletion() async throws {
        guard let client = client else {
            XCTFail("Failed to initialize Ollama client")
            return
        }
        
        let message = LLMRequest.Message(role: "user", content: "Hello, Ollama!")
        let request = LLMRequest(model: "llama2", messages: [message])
        ...
        do {
            let response = try await client.createChatCompletion(request: request) { result in
                ...
                case .failure(let error):
                    XCTFail("Ollama request failed: \(error.localizedDescription)")
                }
            }
