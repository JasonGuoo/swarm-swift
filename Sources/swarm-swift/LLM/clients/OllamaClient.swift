            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
                   let generatedText = json["response"] as? String {
                    let response = LLMResponse(id: "", object: "", created: 0, model: "", choices: [LLMResponse.Choice(index: 0, message: LLMMessage(role: "", content: generatedText), finishReason: nil)], usage: nil, systemFingerprint: nil, error: nil)
    ...
    override func createCompletion(request: LLMRequest, completion: @escaping (Result<LLMResponse, Error>) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: urlRequest) { data, response, error in
            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any],
                   let generatedText = json["response"] as? String {
                    let response = LLMResponse(id: "", object: "", created: 0, model: request.model, choices: [LLMResponse.Choice(index: 0, message: LLMMessage(role: "", content: generatedText), finishReason: nil)], usage: nil, systemFingerprint: nil, error: nil)
                    completion(.success(response))
                } else {
                    completion(.failure(LLMError.decodingFailed))
                }
